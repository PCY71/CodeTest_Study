...
function solution(alp, cop, problems) {
    ...
    let dp = Array.from(Array(max_alp + 2), () => Array(max_cop + 2).fill(Infinity));
    dp[alp][cop] = 0;
            ...
            for (const problem of problems) {
                ...
                if (i >= alp_req && j >= cop_req) {
                    ...
                    if (i + alp_rwd > max_alp && j + cop_rwd > max_cop) {
                        ...
                        dp[max_alp][j + cop_rwd] = Math.min(dp[max_alp][j + cop_rwd], dp[i][j] + cost);
                        //코딩력 초과
                    } else if (j + cop_rwd > max_cop) {
                        dp[i + alp_rwd][max_cop] = Math.min(dp[i + alp_rwd][max_cop], dp[i][j] + cost);
                        //일반 케이스
                    } else {
